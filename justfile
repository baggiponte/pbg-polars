...
@build:
  npx slidev build --base=/pbg-polars
...
[private]
  ...
  #! /usr/bin/env zsh
  if ! gh repo list baggiponte | grep --quiet "/\bpbg-polars\b"; then
    while true; do
      ...
      case $REPLY in
        [Yy])
          ...
          case $PRIVATE in
            [Yy])
              gh repo create baggiponte/pbg-polars --private --source=.
              ;;
            [Nn])
              gh repo create baggiponte/pbg-polars --public --source=.
              ;;
            *)
    ...
    while true; do
      ...
      case $REPLY in
        [Yy])
          git remote add origin git@github.com:baggiponte/pbg-polars
          break
          ;;
        [Nn])
          break
